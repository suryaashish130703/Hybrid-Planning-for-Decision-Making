...
class MemoryManager:
    ...
    def get_historical_context_for_heuristics(self) -> dict:
        """
        Get historical context for heuristic enhancement.
        Returns recent queries and entities.
        """
        context = {
            "previous_queries": [],
            "entities": []
        ...
        # Get recent queries from memory items
        for item in reversed(self.items[-10:]):  # Last 10 items
            if item.type == "run_metadata":
                # Check if user_query exists in metadata or text
                if hasattr(item, 'user_query') and item.user_query:
                    context["previous_queries"].append(item.user_query)
                elif item.metadata and item.metadata.get("user_query"):
                    context["previous_queries"].append(item.metadata.get("user_query"))
            # Extract entities from metadata if available
            if item.metadata and item.metadata.get("entities"):
                context["entities"].extend(item.metadata.get("entities", []))
